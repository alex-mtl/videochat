...
function startSignaling() {
    ...
    ws.onmessage = event => {
        ...
        if (data.type === 'id') {
            ...
            guestTableUpdate(clientId, 'Self');

            console.log('You are joined as:', clientId);

            if(sessionID !== data.room.host.uid) {
                peerConnection = createPeerConnection(data.room.host.uid, true);
                ...
                for(i = 1; i <= data.room.size; i++) {
                    if (data.room.hasOwnProperty('u'+i)) {
                        user = data.room['u'+i];
            ...
            if (sessionID != clientId) {
                roomEnv = data.room;
                console.log("peer : ", clientId, "host :",roomEnv.host.uid)
                isHost = (clientId === roomEnv.host.uid);
                peerConnection = createPeerConnection(clientId, isHost);
                peerConnection.onicecandidate = event => {
                    if (event.candidate) {
                ...
                tRow.appendChild(td1)
                td2 = document.createElement('td');
                td2.textContent =  + isHost ? 'Streamer' : 'Joined';
                tRow.appendChild(td2)
                document.querySelector('table#guests tbody').prepend(tRow)
            ...
            if (roomEnv.host.uid === data.id) {
                console.log('Host stop the stream!')
            }
            roomEnv = data.room;
    ...
    function handleAnswer(answer) {
        ...
        guestTableUpdate(answer.from, 'Connected');
